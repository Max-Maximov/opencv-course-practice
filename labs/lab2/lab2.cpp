...
int main(int argc, char* argv[])
{
	...
	{
		KeyPoint firstPoint = keypoints1[matches[matchInd].queryIdx];
		double dist = sqrt(pow(firstPoint.pt.x - center.x, 2) +
			pow(firstPoint.pt.y - center.y, 2));
		double angle = atan((center.y - firstPoint.pt.y) / (center.x - firstPoint.pt.x));

		KeyPoint secondPoint = keypoints2[matches[matchInd].trainIdx];
		double delta_a = M_PI / 180 * (secondPoint.angle - firstPoint.angle);
		double delta_scale = secondPoint.size / firstPoint.size;
		
		Point2f newCenter(secondPoint.pt.x + delta_scale * dist * cos(delta_a + angle),
			secondPoint.pt.y + delta_scale * dist * sin(delta_a + angle));
	...
	vector<Point2f> first(matches.size()), after(matches.size());

	for (int i = 0; i < matches.size(); i++)
	{
		first[i] = keypoints1[matches[i].queryIdx].pt;
		after[i] = keypoints2[matches[i].trainIdx].pt;
	}

	Mat homography = findHomography(first, after, CV_RANSAC);
	
	Mat warpImage;
	warpPerspective(image, warpImage, homography, scene.size());
